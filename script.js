...
app.post("/create-checkout-session", async (req, res) => {
    ...
    const session = await stripe.checkout.sessions.create({
        payment_method_types: ["FR63 2004 1010 2105 3158 0X01 835"],
        line_items: [{
            price_data: {
                currency: "eur",
                product_data: {
...
function isAvailable(db, moto, start, end) {
  return !db.some(r =>
    r.moto === moto &&
    (new Date(start) <= new Date(r.end) &&
     new Date(end) >= new Date(r.start))
...
app.post("/checkout", async (req, res) => {
  ...
  if (!isAvailable(db, moto, start, end)) {
    return res.json({ error: "Dates déjà prises" });
  }
...
app.listen(3000, () => console.log("🔥 FULL AUTO READY"));
